...
# ROC 곡선 그리기 함수
def plot_roc_curve(fold, all_labels, all_probs, save_dir='../ROCAUC/kobert_v2',MODE = "SAVE"):
...
def report_evaluation(fold, metrics, dataset=None, val_idx=None):
    ...
    if dataset is not None and val_idx is not None:
        ...
        case_dir = f'../case_samples/kobert_v2/fold_{fold}'
    ...
    for train_idx, val_idx in skf.split(np.zeros(len(labels)), labels):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if early_stopping(metrics['avg_loss']):
                report_evaluation(fold, metrics, dataset, val_idx)    #dataset, val_idx
                break
        ...
        model_dir = '../Result/model/kobert_v2'
