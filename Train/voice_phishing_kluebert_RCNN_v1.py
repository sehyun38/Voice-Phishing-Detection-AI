...
def main():
    dataset = VoicePhishingDataset(FILE_PATH, PT_SAVE_PATH, tokenizer, MAX_LENGTH, model_name, USE_TOKEN_WEIGHTS)
    ...
    for train_idx, val_idx in skf.split(np.zeros(len(labels)), labels):
        ...
        model = RCNN(VOCAB_SIZE, embed_size, hidden_size, num_classes).to(DEVICE)

        optimizer = optim.Adam(model.parameters(), lr=2e-5, weight_decay=5e-5)
        early_stopping = DualEarlyStopping(patience=10, min_delta=0.00001, focus='loss')
        ...
        # 현재 fold 줃 최적의 성능(ROC AUC)이라면 모델을 저장 및 갱신
