...
class VoicePhishingDataset(Dataset):
    def __init__(self, use_precomputed_weights=False, precomputed_file=precomputed_weights_path, verbose=False):
        try:
        ...
        for idx, row in self.data.iterrows():
            ...
            if verbose:
                visualize_token_weights(self.samples, tokenizer, verbose=True)
...
def plot_metrics_from_lists(fold,train_metrics, val_metrics, save_path=f'../Result/kobert_v3'):
...
# ROC 곡선 그리기 함수
def plot_roc_curve(fold, all_labels, all_probs, save_dir='../Result/kobert_v3/ROCAUC'):
...
def report_evaluation(fold, metrics, dataset=None, val_idx=None):
    ...
    if dataset is not None and val_idx is not None:
        ...
        case_dir = f'../Result/kobert_v3/case_samples/fold_{fold}'
...
if __name__ == "__main__":
    ...
    for train_idx, val_idx in skf.split(np.zeros(len(labels)), labels):
        ...
        model_dir = '../Result/kobert_v3/model'
